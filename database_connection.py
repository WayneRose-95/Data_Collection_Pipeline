...
log_filename = get_absolute_file_path(
    "database_connection.log", "logs"
)  # "logs/database_utils.log"
if not os.path.exists(log_filename):
    os.makedirs(os.path.dirname(log_filename), exist_ok=True)

database_connection_logger = logging.getLogger(__name__)

# Set the default level as DEBUG
database_connection_logger.setLevel(logging.DEBUG)
...
database_connection_logger.addHandler(file_handler)
...
class DatabaseConnector:
    def read_database_credentials(self, config_file: yaml):
        ...
        database_connection_logger.info(f"Reading {config_file}")
        valid_extensions = [".yaml", ".yml"]
        name_of_file, file_extension = os.path.splitext(config_file)
        database_connection_logger.info("Validating file extension")
        database_connection_logger.info(name_of_file)
        database_connection_logger.info(file_extension)
        if file_extension.lower() not in valid_extensions:
            database_connection_logger.exception(
                "Invalid file extension. Only YAML files are allowed"
            )
            raise ValueError("Invalid file extension. Only YAML files are allowed.")
        # Read the yaml file
        database_connection_logger.info(f"{config_file} successfully validated")
        try:
            database_connection_logger.info(f"Opening {config_file}")
            with open(config_file) as file:
                database_credentials = yaml.safe_load(file)
                database_connection_logger.info(f"Config file {config_file} loaded")
                database_connection_logger.info(database_credentials)
            # Return the yaml file as a dictionary
            return database_credentials
        # If the file is not found, raise an exception
        except FileNotFoundError:
            database_connection_logger.exception(f"Logging file {config_file} not found")
            raise FileNotFoundError("Config file not found")
        # If the config file is not in a YAML format, raise an exception
        except yaml.YAMLError:
            database_connection_logger.exception(f"Invalid file format")
            database_connection_logger.critical(
    ...
    def create_connection_string(
        ...
        database_connection_logger.debug(f"Connection string created: {connection_string}")
    ...
    def initialise_database_connection(
        ...
        try:
            database_connection_logger.info(
            ...
            database_connection_logger.info("connection successful")
